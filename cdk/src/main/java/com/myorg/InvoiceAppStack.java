...
public class InvoiceAppStack extends Stack {
  ...
  public InvoiceAppStack(
    ...
    bucket.addEventNotification(
      ...
      new SnsDestination(s3InvoiceTopic.getTopic())
    );

    Queue s3InvoiceDlqQueue = Queue.Builder
      .create(this, "S3InvoiceDlq")
      .queueName("s3-invoice-events-dlq")
      .build();
    DeadLetterQueue deadLetterQueue = DeadLetterQueue
      .builder()
      .queue(s3InvoiceDlqQueue)
      .maxReceiveCount(3)
      .build();

    this.s3InvoiceQueue =
      Queue.Builder
        .create(this, "S3InvoiceQueue")
        .queueName("s3-invoice-events")
        .deadLetterQueue(deadLetterQueue)
        .build();

    SqsSubscription sqsSubscription = SqsSubscription.Builder
      .create(s3InvoiceQueue)
      .build();
